...
void BlockBuilder::Add(const Slice& key, const Slice& value) {
  ...
  //如果当前重启点已经关联的key数量没有超过阈值（默认阈值为16），计算当前key和重启点key的共同前缀的长度(shared)和当前key独有的后缀长度(non_shared)
  size_t shared = 0;
  ...
  // Update state
  //把last_key设置为当前key，并把counter加1，即把当前重启点关联的key数量加1（重启点key本身也算一个关联key）。
