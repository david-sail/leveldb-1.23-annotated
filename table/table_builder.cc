...
void TableBuilder::Add(const Slice& key, const Slice& value) {
  ...
  assert(!r->closed);
  if (!ok()) return;
  //检查是否存在错误，以及检查本次Add的key是否大于last_key，TableBuilder只接受升序添加key，因为这个是为Memtable服务的，所以key必然是升序的。
  if (r->num_entries > 0) {
    assert(r->options.comparator->Compare(key, Slice(r->last_key)) > 0);
  }

  //r->pending_index_entry is true only if data_block is empty.
  //如果本次key/value是data block的第一个插入;则更新上一个data block的索引信息(保存offset和size),Flush函数计算得到
